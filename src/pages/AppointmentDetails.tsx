...
const AppointmentDetails: React.FC = () => {
  ...
  const updateTimeElapsed = () => {
    if (appointment?.createdAt) {
      ...
      if (diffDays > 0) {
        setTimeElapsed(`Il y a ${diffDays} jour${diffDays > 1 ? 's' : ''}`);
      } else if (diffHours > 0) {
        setTimeElapsed(`Il y a ${diffHours} heure${diffHours > 1 ? 's' : ''}`);
      } else if (diffMins > 0) {
        setTimeElapsed(`Il y a ${diffMins} minute${diffMins > 1 ? 's' : ''}`);
      } else {
  ...
  const calculateConfirmationDelay = () => {
    if (appointment?.createdAt && appointment?.confirmedAt) {
      const submitted = new Date(appointment.createdAt);
      const confirmed = new Date(appointment.confirmedAt);
      const diffMs = confirmed.getTime() - submitted.getTime();
      const diffMins = Math.floor(diffMs / 60000);
      const diffHours = Math.floor(diffMins / 60);

      if (diffHours > 0) {
        const remainingMins = diffMins % 60;
        setConfirmationDelay(`${diffHours}h${remainingMins > 0 ? remainingMins : ''}`);
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Colonne de gauche - 2 colonnes */}
          <div className="lg:col-span-2 space-y-6">
            {/* Carte Médecin et Patient */}
            ...
            {/* Carte Informations du rendez-vous */}
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
              <h2 className="text-xl font-semibold mb-4 flex items-center text-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {appointment.reason && (
                    <div>
                      ...
                      <p className="text-gray-800 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        {appointment.reason}
                      </p>
                    </div>
                  )}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 sticky top-6">
              ...
              <div className="space-y-6">
                {/* Soumission */}
                {createdAt && (
                  <div className="relative pl-8 pb-6 border-l-2 border-blue-200 last:border-l-2">
                    ...
                    <p className="text-sm text-gray-500 mt-2 bg-blue-50 p-2 rounded-lg">
                      {timeElapsed}
                    </p>
                  </div>
                )}
                ...
                {confirmedAt ? (
                  <div className="relative pl-8 pb-6 border-l-2 border-green-200">
                    ...
                    <p className="text-gray-800 font-medium">
                      {format(confirmedAt, 'dd MMM yyyy', { locale: fr })}
                    </p>
                    <p className="text-gray-600">
                      {format(confirmedAt, 'HH:mm')}
                    </p>
                    {confirmationDelay && (
        ...
        {/* Section notation si terminé et non noté */}
