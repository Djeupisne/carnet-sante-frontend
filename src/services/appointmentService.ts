...
export const appointmentService = {
  ...
  async createAppointment(data: CreateAppointmentData): Promise<Appointment> {
    try {
      ...
      // ✅ CORRIGÉ: retour direct, pas .appointment
  ...
  async getAppointments(): Promise<Appointment[]> {
    try {
      ...
      // ✅ CORRIGÉ: data est directement un tableau
   ...
   */
  async rateAppointment(id: string, rating: number, feedback?: string): Promise<void> {
    try {
      console.log('⭐ Notation du rendez-vous:', { id, rating })
      const response = await api.post(`/appointments/${id}/rate`, { rating, feedback })
      
      if (!response.data?.success) {
        throw new Error('Erreur lors de la notation du rendez-vous')
      }
      
      console.log('✅ Rendez-vous noté avec succès')
    } catch (error: any) {
      console.error('❌ Erreur notation rendez-vous:', error)
      throw error
    }
  }
}
